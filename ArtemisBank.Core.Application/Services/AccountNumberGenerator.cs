...
{
    /// <summary>
    /// Genera un identificador de 9 digitos unico en todo el sistema.
    /// Verifica contra cuentas de ahorro (Michael) y contra numeros de prestamo (Manuel).
    /// </summary>
    public class AccountNumberGenerator : IAccountNumberGenerator
    {
        ...
        public async Task<string> GenerateAsync(CancellationToken cancellationToken = default)
        {
            for (var attempt = 0; attempt < MaxAttempts; attempt++)
            {
                ...
                // Se almacena como texto: se permiten ceros a la izquierda.
                int value;
                lock (Randomizer)
                {
